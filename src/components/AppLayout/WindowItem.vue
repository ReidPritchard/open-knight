<template>
  <component :is="wrapperComponent" v-bind="wrapperProps">
    <section
      ...
      :id="window.id"
      :aria-label="`${window.id} window`"
      :aria-expanded="!isCollapsed"
      role="region"
      tabindex="0"
      @keydown.space.prevent="emit('update:toggle-collapse', window.id)"
    >
      <div class="app-layout__window-header">
        <span class="app-layout__window-title">{{ window.id }}</span>
      </div>
      <div
      ...
      <ContextMenu
        ...
        :target="`.app-layout__window#${window.id}`"
      />
    </section>
  </component>
</template>
...
const displayMode = computed(() =>
  isContainer.value.success ? (props.window as IWindowContainer).display : null
);
